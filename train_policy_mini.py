...
from TurnEventTokenizer import build_sequence_vocab, PAD_ID
...
MODEL_VARIANTS: Dict[str, Dict[str, Any]] = {
    "default": {},
    "model2_mini_base": {
        "model_name": "model2_mini_base",
        "hidden_dim": 256,
        "depth": 3,
        "transition_hidden_dim": 256,
        "description": "model2 retrained on 262-dim mini state (no move_hash, no team_comp, reduced species hash).",
    },
    "model2_mini_decoded": {
        "model_name": "model2_mini_decoded",
        "hidden_dim": 256,
        "depth": 3,
        "transition_hidden_dim": 256,
        "description": "model2 joint-policy on 262-dim mini state with turn-outcome auxiliary head.",
    },
    "model_1_augmented": {
        "model_name": "model_1_augmented",
        "hidden_dim": 256,
        "depth": 3,
        "transition_hidden_dim": 256,
        "description": "Model_1 retrained with augmented state (opponent team composition).",
    },
    "model_2_augmented": {
        "model_name": "model_2_augmented",
        "hidden_dim": 256,
        "depth": 3,
        "transition_hidden_dim": 256,
        "description": "Model_2 retrained with augmented state (opponent team composition).",
    },
    "model_2_large": {
...
def use_action_vocab(args: argparse.Namespace) -> bool:
    return bool(args.include_switches or args.predict_turn_outcome or args.predict_turn_sequence)


def requires_training_model(args: argparse.Namespace) -> bool:
    return bool(args.predict_turn_outcome or args.predict_value or args.predict_turn_sequence or args.predict_from_history)
...
def build_policy_models(
    ...
    *,
    ...
    history_lstm_dim: int = 32,
    history_turns: int = 8,
    history_events_per_turn: int = 24,
    history_decode_weight: float = 0.1,
):
    ...
    use_sequence = sequence_vocab_size is not None and sequence_length is not None
    if not use_transition and not predict_value and not use_sequence and not use_history:
    ...
    # History encoder: BiLSTM over past turn event sequences + cross-attention.
    # Aux heads use _aux_shared (history-fused); policy head keeps shared (history-free).
    ...
    if use_history:
        ...
        hist_tokens_input = layers.Input(
            shape=(history_turns, history_events_per_turn), dtype="int32", name="event_history_tokens"
        )
        hist_mask_input = layers.Input(shape=(history_turns,), dtype="float32", name="event_history_mask")
        emb = layers.Embedding(history_vocab_size, history_embed_dim, mask_zero=True)(hist_tokens_input)
        # Mean-pool over event axis, PAD-masked (token==0 is PAD)
        def pool_turns(x):
            emb, tokens = x
            token_mask = tf.cast(tf.not_equal(tokens, 0), tf.float32)
            emb_sum = tf.reduce_sum(emb * tf.expand_dims(token_mask, -1), axis=2)
            cnt = tf.maximum(tf.reduce_sum(token_mask, axis=2, keepdims=True), 1.0)
            return emb_sum / cnt
        turn_rep = layers.Lambda(pool_turns)([emb, hist_tokens_input])
        lstm_out = layers.Bidirectional(
            layers.LSTM(history_lstm_dim, return_sequences=True), name="history_bilstm"
        )(turn_rep)
        attn_dim = history_lstm_dim * 2
        query = layers.Dense(attn_dim, name="hist_attn_query")(shared)
        keys = layers.Dense(attn_dim, name="hist_attn_keys")(lstm_out)
        def cross_attn(x):
            q, k, mask = x
            scores = tf.einsum("bd,bkd->bk", q, k)
            scores = scores + (1.0 - mask) * -1e9
            return tf.nn.softmax(scores, axis=-1)
        attn_weights = layers.Lambda(cross_attn)([query, keys, hist_mask_input])
        def ctx_fn(x):
            attn, lstm = x
            return tf.einsum("bk,bkd->bd", attn, lstm)
        context = layers.Lambda(ctx_fn)([attn_weights, lstm_out])
        fused = layers.Concatenate(name="hist_fuse_concat")([shared, context])
        _aux_shared = layers.Dense(hidden_dim, activation="relu", name="history_fuse")(fused)
    else:
        _aux_shared = shared

    policy_value_model = None
    if predict_value:
        value_x = layers.Dense(value_hidden_dim or max(64, hidden_dim // 2), activation="relu")(_aux_shared)
        if dropout > 0:
    ...
    if use_sequence:
        # Reuse action inputs/embedding from transition head, or create new ones
        if not use_transition:
            my_action_input = layers.Input(shape=(), dtype="int32", name="my_action")
            opp_action_input = layers.Input(shape=(), dtype="int32", name="opp_action")
            action_context_vs = action_context_vocab_size or (sequence_vocab_size + 4)
            action_embedding = layers.Embedding(
                input_dim=action_context_vs,
                output_dim=max(8, action_embed_dim),
                name="action_embedding",
            )

        # Build conditioning: shared state + action embeddings
        seq_context = layers.Concatenate(name="sequence_context")([
            _aux_shared,
            action_embedding(my_action_input),
            action_embedding(opp_action_input),
        ])

        # Repeat context across timesteps -> LSTM -> Dense per timestep
        seq_x = layers.RepeatVector(sequence_length, name="sequence_repeat")(seq_context)
        seq_x = layers.LSTM(sequence_hidden_dim, return_sequences=True, name="sequence_lstm")(seq_x)
        if dropout > 0:
            seq_x = layers.Dropout(dropout)(seq_x)
        sequence_out = layers.TimeDistributed(
            layers.Dense(sequence_vocab_size),
            name="sequence",
        )(seq_x)
    ...
    # History decode head: reconstruct the most recent past turn's event sequence.
    if use_history and lstm_out is not None:
        last_turn = lstm_out[:, -1, :]
        decode_in = layers.Concatenate(name="hist_decode_in")([_aux_shared, last_turn])
        decode_proj = layers.Dense(sequence_hidden_dim, activation="relu", name="hist_decode_proj")(decode_in)
        decode_rep = layers.RepeatVector(history_events_per_turn, name="hist_decode_repeat")(decode_proj)
        decode_lstm = layers.LSTM(sequence_hidden_dim, return_sequences=True, name="hist_decode_lstm")(decode_rep)
        hist_decode_out = layers.TimeDistributed(
            layers.Dense(history_vocab_size, activation="softmax"),
            name="hist_decode",
        )(decode_lstm)
        outputs["hist_decode"] = hist_decode_out
        losses["hist_decode"] = "sparse_categorical_crossentropy"
        loss_weights["hist_decode"] = history_decode_weight
    ...
    if use_history:
        if isinstance(model_inputs, dict):
            ...
            model_inputs = {
                "state": model_inputs,
                "event_history_tokens": hist_tokens_input,
                "event_history_mask": hist_mask_input,
            }
...
def to_numpy_inputs(raw_inputs: Any) -> Any:
    if isinstance(raw_inputs, dict):
        ...
        for key, values in raw_inputs.items():
            if key in ("state", "event_history_mask"):
                dtype = np.float32
            elif key == "event_history_tokens":
                dtype = np.int32
            else:
                dtype = np.int64
...
def make_training_metadata(
    ...
    *,
    ...
    if args.predict_turn_sequence:
        objective_set.append("sequence")
    if args.predict_from_history:
    ...
    payload = {
        ...
        "state_schema_version": "flat_public_mini_v1",
        "state_encoder": "mini",
    ...
    if args.predict_turn_sequence:
        payload["sequence_vocab_size"] = len(sequence_vocab) if sequence_vocab else 0
    ...
    payload["use_history"] = bool(args.predict_from_history)
    if args.predict_from_history:
        payload["history_turns"] = args.history_turns
        payload["history_events_per_turn"] = args.history_events_per_turn
        payload["history_embed_dim"] = args.history_embed_dim
        payload["history_lstm_dim"] = args.history_lstm_dim
        payload["history_decode_weight"] = args.history_decode_weight
        payload["history_schema_version"] = "event_bilstm_crossattn_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sequence-hidden-dim", type=int, default=128,
                        help="LSTM hidden dimension for sequence head.")
    parser.add_argument("--max-seq-len", type=int, default=32,
                        help="Maximum padded sequence length for turn event tokens.")
    parser.add_argument("--predict-from-history", action="store_true", default=False,
                        help="Add history encoder: rolling K-turn event buffer → BiLSTM → cross-attention.")
    parser.add_argument("--history-turns", type=int, default=8,
                        help="Number of past turns to buffer for history encoder.")
    parser.add_argument("--history-events-per-turn", type=int, default=24,
                        help="Max events per buffered turn (padded/clipped).")
    parser.add_argument("--history-embed-dim", type=int, default=16,
                        help="Event token embedding dim for history encoder. Default 16 keeps params ~460k (CPU-trainable).")
    parser.add_argument("--history-lstm-dim", type=int, default=32,
                        help="BiLSTM hidden dim for history encoder. Default 32 keeps params ~460k (CPU-trainable).")
    parser.add_argument("--history-decode-weight", type=float, default=0.1,
                        help="Loss weight for history reconstruction auxiliary head.")
    parser.add_argument(
        "--patience",
        type=int,
        default=3,
...
def main() -> None:
    ...
    sequence_vocab = None
    if args.predict_turn_sequence or args.predict_from_history:
        sequence_vocab = build_sequence_vocab(train_examples)
    needs_action_context = args.predict_turn_outcome or args.predict_turn_sequence
    if use_action_vocab(args):
        ...
        X_raw, targets_raw = vectorize_multitask_dataset(
            ...
            state_encoder=encode_state_mini,
            include_transition=needs_action_context,
    ...
    # hist_decode target: last turn's token ids — taken from the history input itself
    y_hist_decode_np: np.ndarray | None = None
    if args.predict_from_history and isinstance(X, dict) and "event_history_tokens" in X:
        y_hist_decode_np = X["event_history_tokens"][:, -1, :].astype(np.int32)
    ...
    if y_sequence_np is not None:
        y_train_sequence = y_sequence_np[train_idx]
        y_val_sequence = y_sequence_np[val_idx] if len(val_idx) else None
    else:
        y_train_sequence = None
        y_val_sequence = None

    if y_hist_decode_np is not None:
        y_train_hist_decode = y_hist_decode_np[train_idx]
        y_val_hist_decode = y_hist_decode_np[val_idx] if len(val_idx) else None
    else:
        y_train_hist_decode = None
        y_val_hist_decode = None
    ...
    model, policy_model, policy_value_model = build_policy_models(
        ...
        action_context_vocab_size=len(action_context_vocab or {}) if needs_action_context else None,
        action_embed_dim=args.action_embed_dim,
        transition_hidden_dim=args.transition_hidden_dim,
        transition_weight=args.transition_weight,
        predict_value=args.predict_value,
        ...
        value_weight=args.value_weight,
        sequence_vocab_size=len(sequence_vocab) if sequence_vocab else None,
        sequence_length=args.max_seq_len if args.predict_turn_sequence else None,
        sequence_hidden_dim=args.sequence_hidden_dim,
        sequence_weight=args.sequence_weight,
        use_history=args.predict_from_history,
        history_vocab_size=len(sequence_vocab) if args.predict_from_history and sequence_vocab else None,
        history_embed_dim=args.history_embed_dim,
        history_lstm_dim=args.history_lstm_dim,
        history_turns=args.history_turns,
        history_events_per_turn=args.history_events_per_turn,
        history_decode_weight=args.history_decode_weight,
    )
    ...
    if multitask_targets:
        ...
        if y_train_hist_decode is not None:
            train_targets["hist_decode"] = y_train_hist_decode
    ...
    if policy_train_weights is not None:
        if multitask_targets:
            ...
            if args.predict_turn_sequence:
                sample_weights["sequence"] = np.ones(len(y_train_policy), dtype=np.float32)
            if y_train_hist_decode is not None:
                sample_weights["hist_decode"] = np.ones(y_train_hist_decode.shape, dtype=np.float32)
    ...
    if X_val is not None and y_val_policy is not None and len(val_idx):
        if multitask_targets:
            ...
            if y_val_hist_decode is not None:
                val_targets["hist_decode"] = y_val_hist_decode
